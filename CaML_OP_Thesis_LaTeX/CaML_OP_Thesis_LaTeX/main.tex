...
CaML-OP combines gradient boosted prognostic encoding with causal meta learning to estimate individualized treatment effects (ITE) for platinum chemotherapy from routine clinical data. It first trains an XGBoost survival model. Extracts its leaf structure to capture non-linear patterns and then feeds this into an R-Learner and a Doubly Robust Learner to calculate the final averaged ITE.
...
The interface design aligns with the EU AI Act\textquotesingle s high risk framework. Features like per-patient explanations and uncertainty reporting act as design responses to the spirit of the regulation. Making the system AI Act-informed rather than legally deployment-ready.
...
Overlap requires that every patient has positive probability of receiving either treatment given their covariates; this is enforced empirically through propensity trimming. SUTVA requires that no patient\textquotesingle s outcome depends on another patient\textquotesingle s treatment assignment; this is reasonable for individual cancer treatment decisions.
...
The narrative module takes the patient\textquotesingle s covariate values, the estimated ITE with its approximate uncertainty interval, and the top three Effect-SHAP drivers in each direction as prompt inputs. The LLM is instructed to produce a two- to three-sentence clinical summary that accurately represents the ITE sign and magnitude, names the dominant SHAP features and acknowledges uncertainty. No medical knowledge beyond these inputs is permitted in the output.

Each generated narrative passes three automated checks before display: the recommendation sign must match tau-hat; the named features must match the top Effect-SHAP features; and any numeric value cited must fall within 15\% of the model output. The 15\% threshold was selected as a working tolerance and is not theoretically derived. The thesis does not provide an empirical pass-rate or clinician-evaluation of these checks, which is acknowledged as a limitation in Chapter 6.
...
Experiments ran on a standard laptop CPU (Intel Core i7, 16 GB RAM). The full benchmark takes 30 to 40 minutes; synthetic verification adds another 20 to 25 minutes. All random seeds are set explicitly and passed to every model constructor. Re-running either script with the same seeds reproduces all reported tables and figures to numerical precision.

\textbf{Software:} Python 3.11; EconML 0.15; XGBoost 2.x; scikit-learn 1.5; lifelines 0.27; pandas 2.x; NumPy 1.26; SciPy 1.11; matplotlib 3.8; seaborn 0.13. A requirements.txt file is included in the supplementary materials.
...
One pattern runs through the chapter and is worth stating upfront. PEHE rank and AIPW estimated-value rank tell different stories. The S-Learner ranks first on PEHE; CaML-OP ranks first on estimated policy value. Curth et al. (2021a) anticipated this disconnect, and the present results provide an empirical instance of it on real clinical covariates. Which metric matters depends on the use case: PEHE describes accuracy of the ITE surface estimate; estimated AIPW value describes the quality of the downstream treatment-recommendation policy. The two rankings should be considered together.

\section{ITE accuracy and policy quality}\label{ite-accuracy-and-policy-quality}

Table 5.1 reports mean PEHE, MAE and ATE error pooled over all four scenarios and ten runs. CaML-OP ranks third on PEHE (0.161), behind the S-Learner (0.101) and the Causal Forest (0.118). The standalone R- and DR-Learner components rank sixth and seventh (0.241 and 0.266). The ensemble therefore outperforms both of its parts by a substantial margin: 33 percent over the R-Learner and 39 percent over the DR-Learner, with the DR comparison reaching p approximately 0.002 under a Wilcoxon signed-rank test on the 40 per-run values.
...
Table 5.3 reports off-policy AIPW estimated value. CaML-OP attains the highest mean estimated policy value in this benchmark (V = 0.559). The estimator also ranks the treat-all baseline (0.551), the oracle benchmark (0.548), and the DR-Learner (0.547) close to one another and to CaML-OP. The right interpretation is therefore not that CaML-OP is equivalent to knowing the true treatment effects. Rather, under this finite-sample off-policy estimator, CaML-OP\textquotesingle s estimated policy value is not statistically distinguishable from the oracle benchmark (Wilcoxon p = 0.42). The fact that the estimator also places simple policies close to or above the oracle reflects finite-sample variance in the off-policy AIPW estimator at n\_test approximately 137. The S-Learner, which ranks first on PEHE, ranks fifth on this metric (0.545); the Causal Forest, second on PEHE, ranks eighth (0.530).

\setcounter{table}{2}\captionof{table}{Off-policy estimated AIPW value of policy pi(X) = 1\{tau-hat(X) \textgreater{} 0\}, pooled over 4 scenarios x 10 runs. Higher is better. Oracle benchmark = 0.548. Standard deviation across 40 runs reported.}
...
\endlastfoot
CaML-OP & 0.559 & 0.073 \\
TreatAll & 0.551 & 0.085 \\
Oracle & 0.548 & 0.090 \\
DR-Learner & 0.547 & 0.077 \\
S-Learner & 0.545 & 0.077 \\
R-Learner & 0.535 & 0.074 \\
T-Learner & 0.531 & 0.063 \\
Causal Forest & 0.530 & 0.082 \\
X-Learner & 0.526 & 0.069 \\
TreatNone & 0.502 & 0.080 \\
\end{longtable}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{./media/image8.png}
\caption{Off-policy estimated AIPW policy value with 95\% error bars (1.96 x SE). CaML-OP is the leftmost bar in red. The green bar marks the oracle benchmark. The dashed horizontal line marks the oracle value at 0.548. Reference policies TreatAll and TreatNone shown in grey at the right.}
\end{figure}
...
\setcounter{table}{6}\captionof{table}{Synthetic Check 1 - architectural correctness. N = 2000, 8 runs, Scenario 4, standard confounding (eta = 1).}
...
\setcounter{table}{7}\captionof{table}{Synthetic Check 2 - competitiveness under strong confounding. N = 2000, 8 runs, Scenario 4, doubled confounding (eta = 2).}
...
\item
  \textbf{Estimated policy quality.} CaML-OP ranks 1st of 7 on off-policy estimated AIPW value (0.559). Under this estimator, the value is not statistically distinguishable from the oracle benchmark (Wilcoxon p = 0.42). PEHE rank does not predict estimated-value rank.
\item
...
Key results. CaML-OP versus DR-Learner: p approximately 0.002 (39 percent PEHE reduction, significant). CaML-OP versus R-Learner: p approximately 0.131 (33 percent reduction, not significant at alpha = 0.05). CaML-OP versus T-Learner: p \textless{} 0.001. The difference between CaML-OP and the S-Learner is not significant (p \textgreater{} 0.20), consistent with the interpretation in Section 5.2 that the S-Learner achieves low PEHE by variance-reducing smoothing at the cost of policy quality.

For estimated policy value: CaML-OP versus oracle benchmark gives p = 0.42 (not distinguishable under this estimator). CaML-OP versus Causal Forest gives p = 0.031. CaML-OP versus TreatNone gives p \textless{} 0.001.
...
On off-policy AIPW estimated value, CaML-OP ranks first. The estimator also ranks the treat-all baseline and the oracle benchmark close to one another, which means the right interpretation is not that CaML-OP is equivalent to knowing the true treatment effects. The result is more modest: under this finite-sample off-policy estimator, CaML-OP\textquotesingle s estimated policy value is not statistically distinguishable from the oracle benchmark. The estimator itself has non-trivial variance at n\_test approximately 137, which is why TreatAll occasionally ranks above the oracle. CaML-OP\textquotesingle s advantage shows up where the threshold decision is made, which is also where the S-Learner\textquotesingle s smoothing concentrates its errors. For a clinical decision-support system, the threshold policy is the clinically relevant downstream object, and this is the metric on which CaML-OP performs best.
...
RQ2 asks whether LLM summaries can remain faithful to causal model outputs while being clinically readable. The architectural answer is yes, conditional on the automated faithfulness check blocking non-compliant narratives before display. This thesis does not provide an empirical pass-rate, a clinician panel evaluation, or a quantitative analysis of how often the LLM produces hallucinated clinical claims that pass the three automated checks. The architecture demonstrates a workflow design; the empirical performance of the narrative module remains unevaluated and is acknowledged as a meaningful gap for future work.
...
\item
  \textbf{Interval under-coverage.} Empirical coverage of 0.847 against a 0.95 nominal target means the reported intervals are optimistic. The reported interval is an approximate uncertainty interval rather than a formal CI for the ensemble.
\item
  \textbf{Encoder leakage.} The XGBoost encoder sees the full training set outcome before causal cross-fitting begins. Cross-fitting mitigates but does not eliminate the resulting leakage.
\item
  \textbf{LLM narrative empirical evaluation.} No pass-rate, failure-rate or clinician evaluation is reported for the narrative module. The architecture is documented, but the empirical performance is not.
\end{itemize}
